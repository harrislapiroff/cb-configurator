...
Safari extensions must be wrapped in a native macOS (and optionally iOS) app and
distributed through the Mac App Store. This requires **macOS**, **Xcode**, and
...
directory and writes an Xcode project to `safari/`. The `--force` flag means
re-running the command will overwrite `safari/` — commit or back up any manual
Xcode customizations (team, bundle identifier, etc.) before re-running.

The `safari/` directory is not generated by the normal CI build and is not
listed in `.gitignore`. You can choose to commit it (treating it like a React
Native iOS project) or regenerate it on each machine. If you commit it, the
Xcode project will reference extension source files via relative paths, so the
`extension/` and `safari/` directories must remain siblings.

**Configure the Xcode project:**

1. Open `safari/Caller's Box Configurator/Caller's Box Configurator.xcodeproj`
2. In the project editor select the app target, then under **Signing &
...
1. Product → Run to build and launch the wrapper app
